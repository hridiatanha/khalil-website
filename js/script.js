...
const Cart = (() => {
  ...
  function increment(name, price) {
    const it = items.find(i => i.name === name && i.price === parseFloat(price));
    if (it) { it.qty += 1; save(); }
  }
    ...
    items.forEach(i => {
      ...
      row.innerHTML = `
        ...
        <div class="cart-right">
          ...
          <button class="remove-btn" data-action="remove" data-name="${i.name}" data-price="${i.price}" type="button">Remove</button>
        </div>
      `;
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("cart-toggle")?.addEventListener("click", () => { updateCartUI(); openCart(); });
  ...
  document.getElementById("clear-cart")?.addEventListener("click", () => { Cart.clear(); updateCartUI(); });
